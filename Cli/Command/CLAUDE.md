...
This file provides guidance to Claude Code (claude.ai/code) when working in `src/addons/HappyBoard/ConvoPlus/Cli/Command`.

## Scope

This directory belongs to the `HappyBoard/ConvoPlus` add-on (Convo+ by HappyBoard). Relative path inside the add-on: `Cli/Command`.

## Directory Role

This area contains XenForo CLI command classes.

PHP classes here should map to the `HappyBoard\ConvoPlus\Cli\Command` namespace.
