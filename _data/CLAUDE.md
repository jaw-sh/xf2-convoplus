...
This file provides guidance to Claude Code (claude.ai/code) when working in `src/addons/HappyBoard/ConvoPlus/_data`.

## Scope

This directory belongs to the `HappyBoard/ConvoPlus` add-on (Convo+ by HappyBoard). Relative path inside the add-on: `_data`.

## Directory Role

This area contains exported XenForo metadata XML; this is the portable source of truth for add-on configuration.
