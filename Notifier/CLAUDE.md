...
This file provides guidance to Claude Code (claude.ai/code) when working in `src/addons/HappyBoard/ConvoPlus/Notifier`.

## Scope

This directory belongs to the `HappyBoard/ConvoPlus` add-on (Convo+ by HappyBoard). Relative path inside the add-on: `Notifier`.

## Directory Role

This area contains notification dispatchers.

PHP classes here should map to the `HappyBoard\ConvoPlus\Notifier` namespace.
