...
This file provides guidance to Claude Code (claude.ai/code) when working in `src/addons/HappyBoard/ConvoPlus/XF/Pub`.

## Scope

This directory belongs to the `HappyBoard/ConvoPlus` add-on (Convo+ by HappyBoard). Relative path inside the add-on: `XF/Pub`.

## Directory Role

This area contains class extensions for XenForo core classes.

PHP classes here should map to the `HappyBoard\ConvoPlus\XF\Pub` namespace.
